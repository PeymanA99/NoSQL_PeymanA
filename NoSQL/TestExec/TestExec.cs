 ...
 * --------------------
 * ver 2.4 : 08 Nov 15
 * - added query predicates and demos for MT2Q1 and MT4Q4, F2015
 * ver 2.3 : 15 Oct 15
 * - added error handling for file accesses
 * ver 2.2 : 13 Oct 15
...
namespace Project2Starter
{
  ...
  // - supports dispatching one or more tests using command line args:
  //     - 4 8 dispatches just testR4() and testR8()
  //     - no integer args will dispatch all tests
  //     - pause command line arg causes prompt for next and pause
  //       at the end of each dispatched test
  class TestExec
  {
    ...
    //----< demonstrating answer to MT2Q1, F2015 >------------------------

    void DemoMT2Q1()
    {
      "Demonstrating MT2Q1".title('=');
      string testKey = "elem1";
      Write("\n  Query for child keys with string value = \"{0}\"", testKey);

      DBL db = new DBL();
      DBElemL elem1 = new DBElemL("elem1", "MT2Q1 Demo");
      elem1.children.Add("elem2");
      elem1.payload = new PL_ListOfStrings();
      DBElemL elem2 = new DBElemL("elem2", "MT2Q1 Demo");
      elem2.children.Add("elem1");
      elem2.payload = new PL_ListOfStrings();
      DBElemL elem3 = new DBElemL("elem3", "MT2Q1 Demo");
      elem3.children.Add("elem5");
      elem3.payload = new PL_ListOfStrings();
      db.insert("elem1", elem1);
      db.insert("elem2", elem2);
      db.insert("elem3", elem3);
      showDBL(db);

      QueryEngine<string, DBElemL> qe = new QueryEngine<string, DBElemL>(db);
      Func<string, bool> qp = MakeQueryForSpecifiedChild(testKey, db);
      VirtualDB<string, DBElemL> vdb = qe.Query(qp);
      foreach(string key in vdb.Keys())
      {
        Write("\n  {0} matches child of element with key {1}", testKey, key);
      }
      WriteLine();
    }
    ...
    void DemoMT4Q4()
    {
      "Demonstrating MT4Q4".title('=');
      string testKey = "elem1";
      Write("\n  Query for children with no children");

      DBL db = new DBL();
      DBElemL elem1 = new DBElemL("elem1", "MT2Q1 Demo");
      elem1.children.Add("elem3");
      elem1.payload = new PL_ListOfStrings();
      DBElemL elem2 = new DBElemL("elem2", "MT2Q1 Demo");
      elem2.children.Add("elem1");
      elem2.payload = new PL_ListOfStrings();
      ...
      QueryEngine<string, DBElemL> qe = new QueryEngine<string, DBElemL>(db);
      Func<string, bool> qp = MakeQueryForNoGrandChildren(db);
      VirtualDB<string, DBElemL> vdb = qe.Query(qp);
      foreach (string key in vdb.Keys())
      {
        Write("\n  {0} matches element with no grandchildren", key);
      }
      WriteLine();
    }
