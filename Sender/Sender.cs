...
namespace Project4Starter
{
  ...
  public class Sender
  {
    ...
    public string remoteUrl { get; set; } = "http://localhost:8087/CommService";
    ...
    {
      sendQ.enQ(msg);
      //Console.WriteLine(" +++++++++++   one send msg ++++++++++");
      return true;
    }
      ...
      {//delegate
        ...
        { //lamda
          while (true)
          {
            try
            {
              Message smsg = sendQ.deQ(); // deque from sender Q
                          if (smsg.content == "closeSender")
                          {
                                Console.Write("\n  send thread quitting\n\n");
                                break;
                          } // you can block the code to get ride of race condition 
