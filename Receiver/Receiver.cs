      ...
      public class Receiver
  {
    ...
    public bool StartService()
    {
      ...
      try
      {
        host = CreateListener();
        host.Open();
        svc = new CommService(); // create a service let reciver to get access to Q 
      }
      catch (Exception ex)
      {
    ...
    /*
     * - Default service action is to display each received message. <<< is it from both Qs?
    ...
    public Action defaultServiceAction()
    {
      Action serviceAction = () =>
      {
        ...
        while (true)
        {
          ...
          Console.Write("\n  -----> Reciver Received message:");
          Console.Write("\n  sender is {0}", msg.fromUrl);
          Console.Write("\n  content is {0}\n", msg.content);
        
          if (msg.content == "closeReceiver")
            break;  //comment these two lines
        }
      };
      return serviceAction;
    }

      public Func<Stopwatch, string> showTime = (Stopwatch aWatch) =>
      {
      //Write("\n  Msg{0} is runnig {1}\n",msg.ToString(), aWatch.IsRunning);
     ...
     */
    public virtual void serverProcessMessage(Message msg, Stopwatch aWatch)
    {
                  ...
                  string msgelapsedTime = String.Format("{0:00}:{1:00}:{2:00}.{3:00}",
                      ts2.Hours, ts2.Minutes, ts2.Seconds,
                      ts2.Milliseconds / 10);
                  Console.WriteLine("\n >>> Msg travel time is: " + msgelapsedTime);
            }
